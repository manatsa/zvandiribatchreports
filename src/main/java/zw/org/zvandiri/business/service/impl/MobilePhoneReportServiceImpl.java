...
@Service
public class MobilePhoneReportServiceImpl implements MobilePhoneReportService {
    ...
    public List<MobilePhone> get(SearchDTO dto) {
        StringBuilder builder = new StringBuilder("Select Distinct m from MobilePhone m left join fetch m.cadre p ");
        int position = 0;
        ...
        builder.append(" order by m.dateCreated DESC, p.lastName ASC, p.firstName ASC");

        TypedQuery<MobilePhone> query = entityManager.createQuery(builder.toString(), MobilePhone.class);
        if (dto.getProvince() != null) {
